...
parser.add_argument('--trainpath', default='/home/stu5/mvs/datasets/dtu/dtu_train', help='train datapath')
parser.add_argument('--testpath', default='/home/stu5/mvs/datasets/dtu/dtu_train', help='test datapath')
parser.add_argument('--trainlist', default='lists/dtu/train.txt', help='train list')
parser.add_argument('--testlist', default='lists/dtu/train.txt', help='test list')
...
parser.add_argument('--depth_inter_r', type=str, default="4,2,0.5", help='depth_intervals_ratio')
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    ...
                    print(
                        "Epoch {}/{}, Iter {}/{}, test loss = {:.3f}, s2_ce_loss = {:.3f}, s2_fm_loss = {:.3f}, time = {:3f}".format(
                            epoch_idx, args.epochs,
                            batch_idx,
                            len(TestImgLoader), loss,
                            scalar_outputs["s2_ce_loss"],
                            scalar_outputs['s2_fm_loss'],
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        ...
                        print("Epoch {}/{}, Iter {}/{}, train loss = {:.3f}, s2_ce_loss = {:.3f}, s2_fm_loss = {:.3f}, time = {:3f}".format(
                                                                            epoch_idx, args.epochs,
                                                                            batch_idx,
                                                                            len(TestImgLoader), loss,
                                                                            scalar_outputs["s2_ce_loss"],
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    depth_src = sample_cuda["src_depths"]
    ...
    try:
        ...
        loss, epe, ce_loss_stages, fm_loss_stages = model_loss(outputs, depth_gt_ms, depth_src, mask_ms, proj_mats,
    ...
    scalar_outputs = {"loss": loss,
                      "epe": epe,
                      "s0_ce_loss": ce_loss_stages[0],
                      "s1_ce_loss": ce_loss_stages[1],
                      "s2_ce_loss": ce_loss_stages[2],
                      "s0_fm_loss": fm_loss_stages[0],
                      "s1_fm_loss": fm_loss_stages[1],
                      "s2_fm_loss": fm_loss_stages[2],
...
if __name__ == '__main__':
    ...
    # model, optimizer
    model = CTMVSNet(refine=False, ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
                     depth_interals_ratio=[float(d_i) for d_i in args.depth_inter_r.split(",") if d_i],
                     share_cr=args.share_cr,
                     cr_base_chs=[int(ch) for ch in args.cr_base_chs.split(",") if ch],
                     grad_method=args.grad_method)
    model.to(device)
    model_loss = ctmvsnet_loss
