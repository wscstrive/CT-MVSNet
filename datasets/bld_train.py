...
class MVSDataset(Dataset):
    ...
    def read_src_depth(self, filename, src_s1, src_s2, src_s3):
        ## keeping images at its original resolution 576x768
        depth_hr = np.array(read_pfm(filename)[0], dtype=np.float32)
        h, w = depth_hr.shape
        src_s1.append(cv2.resize(depth_hr, (w//4, h//4), interpolation=cv2.INTER_NEAREST))
        src_s2.append(cv2.resize(depth_hr, (w//2, h//2), interpolation=cv2.INTER_NEAREST))
        src_s3.append(depth_hr)
        return src_s1, src_s2, src_s3
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                src_s1, src_s2, src_s3 = self.read_src_depth(depth_filename, src_s1, src_s2, src_s3)
